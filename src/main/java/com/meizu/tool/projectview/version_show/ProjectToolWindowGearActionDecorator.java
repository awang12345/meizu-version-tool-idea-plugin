...
public class ProjectToolWindowGearActionDecorator extends ToolWindowManagerAdapter implements ProjectComponent {
    ...
    private void decorateToolWindow(ToolWindowImpl toolWindow) {
        List<AnAction> actions = getProjectView().map(projectView -> projectView.getActions(false)).orElse(new ArrayList<>());
    ...
    private Optional<ProjectViewImpl> getProjectView() {
        return Optional.of(ProjectViewImpl.getInstance(this.project)).filter(ProjectViewImpl.class::isInstance)
                .map(ProjectViewImpl.class::cast).filter(projectView -> projectView.getContentManager() != null);
    }
    ...
    private Optional<ToolWindowImpl> getToolWindow() {
        ...
        return Optional.of(toolWindowManager).map(ToolWindowManagerEx::getLastActiveToolWindowId).filter("Project"::equals)
                .map(toolWindowManager::getToolWindow).filter(ToolWindowImpl.class::isInstance).map(ToolWindowImpl.class::cast);
    }
