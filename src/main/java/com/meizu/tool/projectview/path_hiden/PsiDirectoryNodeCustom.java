...
public class PsiDirectoryNodeCustom extends PsiDirectoryNode {
    ...
    public PsiDirectoryNodeCustom(Project project, @NotNull PsiDirectory value, ViewSettings viewSettings) {
    ...
    protected void updateImpl(PresentationData data) {
        ...
        if (ProjectRootsUtil.isModuleContentRoot(directoryFile, project)) {
            ...
            if (module != null) {
                ...
                data.setLocationString(ProjectViewDirectoryHelper.getInstance(project)
                        .getLocationString(psiDirectory, shouldShowUrl, this.shouldShowSourcesRoot()));
                this.setupIcon(data, psiDirectory);
                return;
            }
        }
        String name1 = parentValue instanceof Project ?
                psiDirectory.getVirtualFile().getPresentableUrl() :
                ProjectViewDirectoryHelper.getInstance(psiDirectory.getProject())
                        .getNodeName(this.getSettings(), parentValue, psiDirectory);
        if (name1 == null) {
            this.setValue(null);
        } else {
